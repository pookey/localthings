...
def _zone_write(payload, rep, href=None):
    ...
    if kind == 'temperature':
        return (['temperatures', 'indoor', 'vs', '0'],
...
ZONE = Capability(
    ...
    poll_tier='warm',
    entities=(
        EhsZoneClimateDesc(key='zone_climate', translation_key='zone1',
                           rep_fn=_first_mode, write_fn=_zone_write),
    ),
)

# Power is read by the composite ZONE entity above, not given its own entity
# -- a coverage-only cap so discover() reports no gap, the same shape as
# DHW_CONSUMED below.
ZONE_POWER = Capability(href=HREF_ZONE_POWER, poll_tier='warm')

# The temperature resource is likewise read by ZONE for current/target/bounds,
# but unlike power it isn't entity-less: the water-law offset it also carries
# has no place on a climate card and stays its own sensor.
ZONE_TEMPERATURE = Capability(
    ...
    entities=(
        ...
        # fields scope `desired` (which the ZONE climate entity owns), not
        # `offset`.
